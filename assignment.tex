...
\subsubsection{Einfache Variante}
Wir übernehmen für die Berechnung der Verfügbarkeit das vorherige Modell, erweitern es jedoch mit einem Übergang von \enquote{dead} zu \enquote{alive}.\\
\begin{figure}
    \centering
		\begin{tikzpicture}[>=latex]
	        \tikzstyle{markov} = [top color=blue!30, bottom color=white, 
	        ...
	        \node[markov,bad,circle,label=below right:{\textbf{b}}] (2)
	        [right=5 of 1]{dead};
	        \draw[->] (1) to[bend left=20] node[sloped,above]{$3\lambda_R + \lambda_N$}(2);
	        \draw[->] (2) to[bend left=20] node[sloped,above]{$\mu$}(1);
	    \end{tikzpicture}
    \caption{Zustände einfache Variante Availability}
    \label{fig:states_simple_availability}
\end{figure}
\subsubsection{Redundante Variante}
Bei der redundanten Variante fügen wir ebenfalls ein Übergang von \enquote{dead} zum Startzustand ein. Für die Berechnung der Verfügbarkeit definieren wir, dass das System nur in den blauen Zuständen verfügbar ist.\\
\begin{figure}
    \centering
        \begin{tikzpicture}[>=latex]
            \tikzstyle{markov} = [top color=blue!30, bottom color=white, 
            ...
            \draw[->] (31) to node[sloped,above]{$3\lambda_R+\lambda_N$}(d);
            \draw[->] (41) to[bend right=20] node[sloped,above]{$\lambda_N$}(d);
            \draw[->] (d)  to[bend left=100] node[sloped,above]{$\mu$} (42);
        \end{tikzpicture}
    \caption{Zustände redundante Variante Availability}
    \label{fig:states_redundancy_availability}
\end{figure}
\section{Ergebnisse}
\begin{figure}
\centering
